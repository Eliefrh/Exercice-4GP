...
public class MouvementDeplacement : MonoBehaviour
{
    ...
    private Coroutine _deplacement; // On conserve une référence de la coroutine pour pouvoir l'arrêter.
    ...
    private Vector3? DeterminerClic()
    {
        ...
        if (Physics.Raycast(ray, out hit) && hit.collider == colliderPlan)
        {
    ...
    private IEnumerator DeplacerCube(Vector3 positionFinale)
    {
        ...
        float distance = Vector3.Distance(positionInitiale, positionFinale);
        float tempsDeplacement = distance / vitesse;
        float tempsEcoulé = 0;

        while (tempsEcoulé <= tempsDeplacement)
        {
            Vector3 nouvellePosition = Vector3.Lerp(positionInitiale, positionFinale, tempsEcoulé / tempsDeplacement);
            transform.position = nouvellePosition;
            tempsEcoulé += Time.deltaTime;
            yield return null;
        }
...
//        {
//            Vector3 nouvellePosition = Vector3.Lerp(positionInitiale,positionFinale, pourcentage);
//            transform.position = nouvellePosition;
